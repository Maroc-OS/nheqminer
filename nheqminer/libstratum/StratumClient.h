...
template <typename Miner, typename Job, typename Solution> class StratumClient {
public:
  StratumClient(std::shared_ptr<boost::asio::io_service> io_s, Miner *m,
                string const &host, string const &port, string const &user,
                string const &pass, int const &retries, int const &worktimeout);
  ~StratumClient() {}

  void setFailover(string const &host, string const &port);
  void setFailover(string const &host, string const &port, string const &user,
                   string const &pass);
...
// ZcashStratumClient
typedef StratumClient<ZcashMiner, ZcashJob, EquihashSolution>
    ZcashStratumClient;
