...
string SanitizeString(const string &str) {
  /**
   * safeChars chosen to allow simple messages/URLs/email addresses, but avoid
   * anything
   * even possibly remotely dangerous like & or >
   */
  static string safeChars("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
                          "01234567890 .,;_/:?@()");
  string strResult;
...
string SanitizeFilename(const string &str) {
  /**
   * safeChars chosen to restrict filename, keeping it simple to avoid
   * cross-platform issues.
   * http://stackoverflow.com/a/2306003
   */
  static string safeChars(
...
string DecodeBase64(const string &str) {
  ...
  return (vchRet.size() == 0) ? string()
                              : string((const char *)&vchRet[0], vchRet.size());
}
...
bool ParseInt32(const std::string &str, int32_t *out) {
  ...
  // over/underflow
  // we still have to check that the returned value is within the range of an
  // *int32_t*. On 64-bit
