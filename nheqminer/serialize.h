 ...
 */
template <class T, class TAl> inline T *begin_ptr(std::vector<T, TAl> &v) {
 ...
 * over
 * "SerializationOp" template, which implements the body of each class'
 * serialization
 * code. Adding "ADD_SERIALIZE_METHODS" in the body of the class causes these
 * wrappers to be
 * added as members.
 */
...
public:
  ...
  template <typename Stream> void Serialize(Stream &s, int, int) const {
    WriteVarInt<Stream, I>(s, n);
  }

  template <typename Stream> void Unserialize(Stream &s, int, int) {
 ...
 */
template <typename T>
unsigned int GetSerializeSize(const boost::optional<T> &item, int nType,
                              int nVersion);
template <typename Stream, typename T>
void Serialize(Stream &os, const boost::optional<T> &item, int nType,
               int nVersion);
template <typename Stream, typename T>
...
unsigned int GetSerializeSize(const boost::array<T, N> &item, int nType,
                              int nVersion);
template <typename Stream, typename T, std::size_t N>
void Serialize(Stream &os, const boost::array<T, N> &item, int nType,
               int nVersion);
template <typename Stream, typename T, std::size_t N>
 ...
 */
template <typename T, typename A>
unsigned int GetSerializeSize(const std::list<T, A> &m, int nType,
                              int nVersion);
template <typename Stream, typename T, typename A>
void Serialize(Stream &os, const std::list<T, A> &m, int nType, int nVersion);
template <typename Stream, typename T, typename A>
void Unserialize(Stream &is, std::list<T, A> &m, int nType, int nVersion);
...
void Serialize_impl(Stream &os, std::vector<T, A> &v, int nType, int nVersion,
  ...
  /*for (typename std::vector<T, A>::iterator vis = v.begin(); vis != v.end();
     ++vis)
      v.push_back(vis);
      std::cout << vis << "vis:" << std::endl;

      std::cout << v << "v:" << std::endl;
      ::Serialize(os, v, nType, nVersion);*/
}
...
template <typename Stream, typename T>
void Unserialize(Stream &is, boost::optional<T> &item, int nType,
                 int nVersion) {
  unsigned char discriminant = 0x00;
...
template <typename Stream, typename T, std::size_t N>
void Unserialize(Stream &is, boost::array<T, N> &item, int nType,
                 int nVersion) {
  for (size_t i = 0; i < N; i++) {
 ...
 */
template <typename T, typename A>
unsigned int GetSerializeSize(const std::list<T, A> &l, int nType,
                              int nVersion) {
...
template <typename Stream, typename T>
inline void SerReadWrite(Stream &s, const T &obj, int nType, int nVersion,
                         CSerActionSerialize ser_action) {
  ::Serialize(s, obj, nType, nVersion);
}

template <typename Stream, typename T>
inline void SerReadWrite(Stream &s, T &obj, int nType, int nVersion,
