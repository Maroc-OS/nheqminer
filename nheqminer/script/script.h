...
class CScriptNum {
   ...
   * integers.
   * The semantics are subtle, though: operands must be in the range [-2^31
   * +1...2^31 -1],
   * but results may overflow (and are valid as long as they are not used in a
   * subsequent
   * numeric operation). CScriptNum enforces those semantics by storing results
   * as
   * an int64 and allowing out-of-range values to be returned as a vector of
   * bytes but
   * throwing an exception if arithmetic is done or the result is interpreted as
   * an integer.
   */
public:
  ...
  static std::vector<unsigned char> serialize(const int64_t &value) {
    ...
    //    - If the most significant byte is < 0x80 and the value is negative,
    //    add
    //    0x80 to it, since it will be subtracted and interpreted as a negative
    //    when
...
public:
  ...
  CScript &operator<<(const CScript &b) {
    ...
    // member fn
    assert(!"Warning: Pushing a CScript onto a CScript with << is probably not "
            "intended, use + to concatenate!");
    return *this;
  }
