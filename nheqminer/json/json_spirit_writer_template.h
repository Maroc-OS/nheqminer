...
String_type non_printable_to_string(unsigned int c) {
  // Silence the warning: typedef ‘Char_type’ locally defined but not used
...
template <class String_type> String_type add_esc_chars(const String_type &s) {
...
//
template <class Value_type, class Ostream_type> class Generator {
...
public:
  Generator(const Value_type &value, Ostream_type &os, bool pretty)
      : os_(os), indentation_level_(0), pretty_(pretty) {
    output(value);
  }
...
private:
  ...
  void output(const Array_type &arr) { output_array_or_obj(arr, '[', ']'); }
  ...
  void output(const String_type &s) { os_ << '"' << add_esc_chars(s) << '"'; }
