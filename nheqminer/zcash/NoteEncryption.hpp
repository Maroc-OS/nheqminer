...
template <size_t MLEN> class NoteEncryption {
protected:
...
public:
  ...
  Ciphertext encrypt(const uint256 &pk_enc, const Plaintext &message);
...
template <size_t MLEN> class NoteDecryption {
protected:
  enum { CLEN = MLEN + NOTEENCRYPTION_AUTH_BYTES };
  uint256 sk_enc;
  uint256 pk_enc;
...
public:
  ...
  Plaintext decrypt(const Ciphertext &ciphertext, const uint256 &epk,
                    const uint256 &hSig, unsigned char nonce) const;

  friend inline bool operator==(const NoteDecryption &a,
                                const NoteDecryption &b) {
