...
void write_bigint(base_blob<8 * LIMBS * sizeof(mp_limb_t)> &blob,
                  const bigint<LIMBS> &val) {
  auto ptr = blob.begin();
...
template <mp_size_t LIMBS>
bigint<LIMBS>
read_bigint(const base_blob<8 * LIMBS * sizeof(mp_limb_t)> &blob) {
  bigint<LIMBS> ret;
...
template <> Fq::Fq(curve_Fq element) : data() {
  write_bigint<4>(data, element.as_bigint());
}

template <> curve_Fq Fq::to_libsnark_fq() const {
...
template <> Fq2::Fq2(curve_Fq2 element) : data() {
  write_bigint<8>(data, fq2_to_bigint(element));
}

template <> curve_Fq2 Fq2::to_libsnark_fq2() const {
...
template <> CompressedG1::CompressedG1(curve_G1 point) {
  if (point.is_zero()) {
...
template <> curve_G1 CompressedG1::to_libsnark_g1() const {
  curve_Fq x_coordinate = x.to_libsnark_fq<curve_Fq>();

  // y = +/- sqrt(x^3 + b)
  auto y_coordinate =
      ((x_coordinate.squared() * x_coordinate) + alt_bn128_coeff_b).sqrt();
...
template <> CompressedG2::CompressedG2(curve_G2 point) {
  if (point.is_zero()) {
    throw std::domain_error("curve point is zero");
  }
...
template <> curve_G2 CompressedG2::to_libsnark_g2() const {
  auto x_coordinate = x.to_libsnark_fq2<curve_Fq2>();

  // y = +/- sqrt(x^3 + b)
  auto y_coordinate =
      ((x_coordinate.squared() * x_coordinate) + alt_bn128_twist_coeff_b)
          .sqrt();
...
template <> ZCProof::ZCProof(const r1cs_ppzksnark_proof<curve_pp> &proof) {
...
template <> r1cs_ppzksnark_proof<curve_pp> ZCProof::to_libsnark_proof() const {
...
bool ProofVerifier::check(
  ...
  if (perform_verification) {
    return r1cs_ppzksnark_online_verifier_strong_IC<curve_pp>(
