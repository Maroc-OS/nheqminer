...
API::~API() { m_acceptor.close(); }

bool API::start(int local_port) {
  boost::asio::ip::tcp::endpoint endp(
      boost::asio::ip::address_v4::from_string("127.0.0.1"), local_port);
  ...
  if (ec) {
    BOOST_LOG_CUSTOM(info) << "Failed to bind local port " << local_port
                           << " err: " << ec;
    return false;
  }
...
void Client::Start() {
  boost::asio::async_read_until(
      m_socket, m_response_buffer, "\n",
      boost::bind(&Client::ReadResponse, shared_from_this(),
                  boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}

void Client::ReadResponse(const boost::system::error_code &ec,
