...
TransactionSignatureCreator::TransactionSignatureCreator(
    ...
    : BaseSignatureCreator(keystoreIn), txTo(txToIn), nIn(nInIn),
      nHashType(nHashTypeIn), checker(txTo, nIn) {}
...
static bool Sign1(const CKeyID &address, const BaseSignatureCreator &creator,
                  const CScript &scriptCode, CScript &scriptSigRet) {
 ...
 * can't be signed),
 * unless whichTypeRet is TX_SCRIPTHASH, in which case scriptSigRet is the
 * redemption script.
 * Returns false if scriptPubKey could not be completely satisfied.
 */
static bool SignStep(const BaseSignatureCreator &creator,
                     const CScript &scriptPubKey, CScript &scriptSigRet,
...
bool ProduceSignature(const BaseSignatureCreator &creator,
                      const CScript &fromPubKey, CScript &scriptSig) {
  ...
  if (whichType == TX_SCRIPTHASH) {
    ...
    txnouttype subType;
    bool fSolved = SignStep(creator, subscript, scriptSig, subType) &&
  ...
  // Test solution
  return VerifyScript(scriptSig, fromPubKey, STANDARD_SCRIPT_VERIFY_FLAGS,
                      creator.Checker());
}
...
static CScript CombineSignatures(const CScript &scriptPubKey,
  ...
  case TX_SCRIPTHASH:
    if (sigs1.empty() || sigs1.back().empty())
      ...
      sigs1.pop_back();
      sigs2.pop_back();
      CScript result = CombineSignatures(pubKey2, checker, txType2, vSolutions2,
                                         sigs1, sigs2);
      result << spk;
      return result;
    }
...
CScript CombineSignatures(const CScript &scriptPubKey, const CTransaction &txTo,
                          unsigned int nIn, const CScript &scriptSig1,
...
CScript CombineSignatures(const CScript &scriptPubKey,
  ...
  EvalScript(stack1, scriptSig1, SCRIPT_VERIFY_STRICTENC,
             BaseSignatureChecker());
  vector<valtype> stack2;
  EvalScript(stack2, scriptSig2, SCRIPT_VERIFY_STRICTENC,
             BaseSignatureChecker());

  return CombineSignatures(scriptPubKey, checker, txType, vSolutions, stack1,
                           stack2);
}
