...
std::vector<unsigned char> GetMinimalFromIndices(std::vector<eh_index> indices,
  ...
  for (std::string::size_type i = 0; i < indices.size(); i++) {
...
void static ZcashMinerThread(ZcashMiner *miner, int size, int pos,
  ...
  miner->NewJob.connect(
      NewJob_t::slot_type([&m_zmt, &header, &space, &offset, &inc, &target,
                           &workReady, &cancelSolver, pos, &pauseMining, &jobId,
                           &nTime](const ZcashJob *job) mutable {
        ...
        if (job) {
          ...
          .track_foreign(
              m_zmt)); // So the signal disconnects when the mining thread exits
  ...
  try {
    ...
    while (true) {
      ...
      // Start working
      while (true) {
        BOOST_LOG_CUSTOM(debug, pos) << "Running Equihash solver with nNonce = "
                                     << nonce.ToString();
        ...
        std::function<void(const std::vector<uint32_t> &, size_t,
                           const unsigned char *)>
            solutionFound = [&actualHeader, &bNonce, &actualTarget, &miner, pos,
                             &actualJobId, &actualTime, &actualNonce1size](
                const std::vector<uint32_t> &index_vector, size_t cbitlen,
              ...
              if (compressed_sol) {
                ...
                actualHeader.nSolution =
                    GetMinimalFromIndices(index_vector, cbitlen);
              }
              speed.AddSolution();
              ...
              if (UintToArith256(headerhash) > actualTarget) {
                BOOST_LOG_CUSTOM(debug, pos) << "Too large: "
                                             << headerhash.ToString();
                return;
              }
              ...
              // Found a solution
              BOOST_LOG_CUSTOM(debug, pos)
                  << "Found solution with header hash: "
                  << headerhash.ToString();
              EquihashSolution solution{bNonce, actualHeader.nSolution,
                                        actualTime, actualNonce1size};
        ...
        std::function<void(void)> hashDone = []() { speed.AddHash(); };
...
void ZcashJob::setTarget(std::string target) {
  if (target.size() > 0) {
    ...
    serverTarget = UintToArith256(uint256S(
...
void ZcashMiner::start() {
  ...
  std::sort(solvers.begin(), solvers.end(),
            [](const ISolver *a, const ISolver *b) {
  ...
  for (std::string::size_type i = 0; i < solvers.size(); ++i) {
    minerThreadActive[i] = true;
    minerThreads[i] = std::thread(
        boost::bind(&ZcashMinerThread, this, nThreads, i, solvers[i]));
    if (solvers[i]->GetType() == SolverType::CPU) {
      ...
      if (!SetThreadPriority(hThread, THREAD_PRIORITY_LOWEST)) {
        ...
        BOOST_LOG_CUSTOM(debug, i) << "Priority set to "
...
// todo: linux set low priority
  ...
  //{
  //    minerThreadActive[i] = true;
  //    minerThreads[i] = std::thread(boost::bind(&ZcashMinerThread<CPUSolver,
  //    CUDASolver, OPENCLSolver, CUDASolver>,
  //        this, nThreads, i, *cuda_contexts.at(i - cpu_contexts.size())));
  //}

  //
  // for (; i < (cpu_contexts.size() + cuda_contexts.size() +
  // opencl_contexts.size()); ++i)
  //{
  //    minerThreadActive[i] = true;
  //    minerThreads[i] = std::thread(boost::bind(&ZcashMinerThread<CPUSolver,
  //    CUDASolver, OPENCLSolver, OPENCLSolver>,
  //        this, nThreads, i, *opencl_contexts.at(i - cpu_contexts.size() -
  //        cuda_contexts.size())));
  //}

  ///*minerThreads = new boost::thread_group();
  // for (int i = 0; i < nThreads; i++) {
  //    minerThreads->create_thread(boost::bind(&ZcashMinerThread, this,
  //    nThreads, i));
  //}*/
...
ZcashJob *ZcashMiner::parseJob(const Array &params) {
  ...
  if (ret->header.nVersion == 4) {
    ...
    ssHeader
        << params[1].get_str() << params[2].get_str() << params[3].get_str()
        << params[4].get_str() << params[5].get_str() << params[6].get_str()
        // Empty nonce
    ...
    try {
      ...
      throw std::logic_error(
          "ZcashMiner::parseJob(): Invalid block header parameters");
    }
...
void ZcashMiner::setJob(ZcashJob *job) { NewJob(job); }

void ZcashMiner::onSolutionFound(
    const std::function<bool(const EquihashSolution &, const std::string &)>
        callback) {
...
void ZcashMiner::acceptedSolution(bool stale) { speed.AddShareOK(); }
...
bool benchmark_solve_equihash(const CBlock &pblock,
  ...
  std::function<void(const std::vector<uint32_t> &, size_t,
      ...
      solutionFound = [&pblock, &nonce](
        ...
        BOOST_LOG_TRIVIAL(debug) << "Solution found, header = "
...
void Solvers_doBenchmark(int hashes, const std::vector<ISolver *> &solvers) {
  ...
  for (ISolver *solver : solvers) {
    if (solver->GetType() == SolverType::CPU) {
      ...
      BOOST_LOG_TRIVIAL(info) << "Benchmarking OPENCL worker ("
                              << solver->getname() << ") "
                              << solver->getdevinfo();
    }
  }
  ...
  BOOST_LOG_TRIVIAL(info) << "Speed: "
                          << ((double)hashes_done * 1000 / (double)msec)
                          << " I/s";
  BOOST_LOG_TRIVIAL(info) << "Speed: "
