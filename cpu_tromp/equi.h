...
void setheader(blake2b_state *ctx, const char *header, const u32 headerLen,
               const char *nce, const u32 nonceLen) {
  uint32_t le_N = WN;
  uint32_t le_K = WK;
...
const char *errstr[] = {"OK", "duplicate index", "indices out of order",
                        "nonzero xor"};
...
int verify(u32 indices[PROOFSIZE], const char *header, const u32 headerlen,
