...
double Speed::Get(std::vector<time_point> &buffer, std::mutex &mutex) {
  ...
  if (past < m_start) {
    interval = ((double)std::chrono::duration_cast<std::chrono::milliseconds>(
                    now - m_start)
                    .count()) /
               1000;
    past = m_start;
  ...
  mutex.lock();
  for (std::vector<time_point>::iterator it = buffer.begin();
       it != buffer.end();) {
    if ((*it) < past) {
      it = buffer.erase(it);
    } else {
...
void Speed::AddHash() { Add(m_buffer_hashes, m_mutex_hashes); }

double Speed::GetHashSpeed() { return Get(m_buffer_hashes, m_mutex_hashes); }

void Speed::AddSolution() { Add(m_buffer_solutions, m_mutex_solutions); }

double Speed::GetSolutionSpeed() {
  return Get(m_buffer_solutions, m_mutex_solutions);
}

void Speed::AddShare() { Add(m_buffer_shares, m_mutex_shares); }

double Speed::GetShareSpeed() { return Get(m_buffer_shares, m_mutex_shares); }

void Speed::AddShareOK() { Add(m_buffer_shares_ok, m_mutex_shares_ok); }
