...
uint256 PRF(bool a, bool b, bool c, bool d, const uint252 &x,
  ...
  blob[0] &= 0x0F;
  blob[0] |=
      (a ? 1 << 7 : 0) | (b ? 1 << 6 : 0) | (c ? 1 << 5 : 0) | (d ? 1 << 4 : 0);
...
uint256 PRF_addr_a_pk(const uint252 &a_sk) { return PRF_addr(a_sk, 0); }

uint256 PRF_addr_sk_enc(const uint252 &a_sk) { return PRF_addr(a_sk, 1); }
