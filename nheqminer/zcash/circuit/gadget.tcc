...
private:
  ...
  boost::array<std::shared_ptr<digest_variable<FieldT>>, NumInputs>
      zk_input_nullifiers;
  boost::array<std::shared_ptr<digest_variable<FieldT>>, NumInputs>
      zk_input_macs;
  boost::array<std::shared_ptr<digest_variable<FieldT>>, NumOutputs>
  ...
  // Input note gadgets
  boost::array<std::shared_ptr<input_note_gadget<FieldT>>, NumInputs>
      zk_input_notes;
  boost::array<std::shared_ptr<PRF_pk_gadget<FieldT>>, NumInputs>
      zk_mac_authentication;

  // Output note gadgets
  boost::array<std::shared_ptr<output_note_gadget<FieldT>>, NumOutputs>
      zk_output_notes;
...
public:
  ...
  joinsplit_gadget(protoboard<FieldT> &pb) : gadget<FieldT>(pb) {
    // Verification
    {
      ...
      unpacker.reset(new multipacking_gadget<FieldT>(
          pb, zk_unpacked_inputs, zk_packed_inputs, FieldT::capacity(),
          "unpacker"));
    }
    ...
    for (size_t i = 0; i < NumInputs; i++) {
      ...
      zk_input_notes[i].reset(new input_note_gadget<FieldT>(
          pb, ZERO, zk_input_nullifiers[i], *zk_merkle_root));

      // The input keys authenticate h_sig to prevent
      // malleability.
      zk_mac_authentication[i].reset(new PRF_pk_gadget<FieldT>(
          pb, ZERO, zk_input_notes[i]->a_sk->bits, zk_h_sig->bits,
          i ? true : false, zk_input_macs[i]));
    }

    for (size_t i = 0; i < NumOutputs; i++) {
      zk_output_notes[i].reset(new output_note_gadget<FieldT>(
          pb, ZERO, zk_phi->bits, zk_h_sig->bits, i ? true : false,
  ...
  void generate_r1cs_constraints() {
    ...
    // Value balance
    {
      ...
      this->pb.add_r1cs_constraint(
          r1cs_constraint<FieldT>(1, left_side, right_side));

      // #854: Ensure that left_side is a 64-bit integer.
      for (size_t i = 0; i < 64; i++) {
        generate_boolean_r1cs_constraint<FieldT>(this->pb, zk_total_uint64[i],
                                                 "");
      }
  ...
  void generate_r1cs_witness(const uint252 &phi, const uint256 &rt,
                             const uint256 &h_sig,
...

    {
      ...
      zk_total_uint64.fill_with_bits(this->pb,
    ...
    for (size_t i = 0; i < NumInputs; i++) {
      ...
      zk_input_notes[i]->generate_r1cs_witness(merkle_path, inputs[i].key,
                                               inputs[i].note);
  ...
  static r1cs_primary_input<FieldT>
  witness_map(const uint256 &rt, const uint256 &h_sig,
    ...
    auto verify_field_elements =
        pack_bit_vector_into_field_element_vector<FieldT>(verify_inputs);
