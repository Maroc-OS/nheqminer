...
template <typename T> void saveToFile(std::string path, T &obj) {
  LOCK(cs_ParamsIO);
...
void loadFromFile(std::string path, boost::optional<T> &objIn) {
  ...
  if (!fh.is_open()) {
    throw std::runtime_error(
        (boost::format("could not load param file at %s") % path).str());
  }
...
public:
  ...
  boost::optional<r1cs_ppzksnark_processed_verification_key<ppzksnark_ppT>>
      vk_precomp;
  ...
  void setProvingKeyPath(std::string path) { pkPath = path; }
  ...
  bool verify(const ZCProof &proof, ProofVerifier &verifier,
              ...
              uint64_t vpub_old, uint64_t vpub_new, const uint256 &rt) {
    if (!vk || !vk_precomp) {
    ...
    try {
      auto r1cs_proof =
          proof.to_libsnark_proof<r1cs_ppzksnark_proof<ppzksnark_ppT>>();

      uint256 h_sig = this->h_sig(randomSeed, nullifiers, pubKeyHash);

      auto witness =
          joinsplit_gadget<FieldT, NumInputs, NumOutputs>::witness_map(
              rt, h_sig, macs, nullifiers, commitments, vpub_old, vpub_new);
  ...
  prove(const boost::array<JSInput, NumInputs> &inputs,
        ...
        uint256 &out_randomSeed, boost::array<uint256, NumInputs> &out_macs,
        boost::array<uint256, NumInputs> &out_nullifiers,
        boost::array<uint256, NumOutputs> &out_commitments, uint64_t vpub_old,
        uint64_t vpub_new, const uint256 &rt, bool computeProof) {
    if (computeProof && !pk) {
    ...
    protoboard<FieldT> pb;
    {
      joinsplit_gadget<FieldT, NumInputs, NumOutputs> g(pb);
      g.generate_r1cs_constraints();
      g.generate_r1cs_witness(phi, rt, h_sig, inputs, out_notes, vpub_old,
                              vpub_new);
    }
    ...
    return ZCProof(r1cs_ppzksnark_prover<ppzksnark_ppT>(
        *pk, primary_input, aux_input, pb.constraint_system));
  }
};
...
uint256 JoinSplit<NumInputs, NumOutputs>::h_sig(
  ...
  const unsigned char
      personalization[crypto_generichash_blake2b_PERSONALBYTES] = {
          'Z', 'c', 'a', 's', 'h', 'C', 'o', 'm',
          'p', 'u', 't', 'e', 'h', 'S', 'i', 'g'};
  ...
  if (crypto_generichash_blake2b_salt_personal(output.begin(), 32, &block[0],
                                               block.size(), NULL, 0, // No key.
                                               NULL, // No salt.
                                               personalization) != 0) {
...
JSInput::JSInput()
    : witness(ZCIncrementalMerkleTree().witness()), key(SpendingKey::random()) {
